...
(defn in-string? [state pos]
  (get #{"StringContent" "String"} (n/name (n/tree state pos))))
...
(#?(:squint defn
   ...
   - when deleting an opening bracket of an empty list, removes both brackets"
                  ...
                  (when-not (and (= 1 (.. state -selection -ranges -length))
                                 (let [^js range (#?(:squint get-in
                                                     :cljs j/get-in) state [:selection :ranges 0])]
                                   (and (.-empty range) (= 0 (.-from range)))))
                    (u/update-ranges state
                                     #js{:annotations (u/user-event-annotation "delete")}
                                     (#?(:squint fn
                                         :cljs j/fn) [^:js {:as range :keys [head empty anchor]}]
                                                     (#?(:squint let
                                                         :cljs j/let)
                                                      [^:js {:as _range from :from to :to} (from-to head anchor)
...
(defn handle-open [^EditorState state ^string open]
  (let [^string close (coll-pairs open)]
    (u/update-ranges state
                     ...
                     (#?(:squint fn
                         :cljs j/fn)
                      [^:js {:keys [from to head anchor empty]}]
                      (cond
...
(defn handle-close [state key-name]
  (u/update-ranges state
                   #js{:annotations (u/user-event-annotation "input")}
                   (#?(:squint fn
                       :cljs j/fn) [^:js {:as _range :keys [empty head from to]}]
...
(#?(:squint defn
    :cljs j/defn) handle-backspace-cmd [^:js {:as view :keys [state]}]
                  (u/dispatch-some view (handle-backspace state)))

(defn handle-open-cmd [key-name]
  (#?(:squint fn
      :cljs j/fn) [^:js {:as view :keys [state]}]
